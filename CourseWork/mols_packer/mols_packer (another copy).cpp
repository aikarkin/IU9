...
void generateSCLattice(MolPackingParams &params) {
    ...
    boost::tuple<glm::vec3, boost::tuple<float, float, float>> rect_shell = mol_struct->GetRectangularShell();

    std::cout << "rectangular shell:" << std::endl;
    std::cout << "\tapposition point: " << vec_to_string(rect_shell.get<0>()) << std::endl;
    boost::tuple<float, float, float> sizes = rect_shell.get<1>();
    std::cout << "\tshell size: " << sizes.get<0>() << "x" << sizes.get<1>() << "x" << sizes.get<2>() << std::endl;
