...
MolCubeShell::MolCubeShell(Molecule *molecule) : mol(molecule) {
    ...
    /*std::cout << "part: " << part << " eps:" << eps << std::endl;*/

    if (mol->AtomsCount() != 3) {
        std::cerr << "error: unable to pack mol. Atoms count should be 3" << std::endl;
        throw;
    }
    ...
    /*std::cout << "v_rot_alpha: " << vec_to_string(v_rot_alpha) << std::endl;
    std::cout << "v_rot_beta: " << vec_to_string(v_rot_beta) << std::endl;
    std::cout << "v_rot_gamma: " << vec_to_string(v_rot_gamma) << std::endl;*/
    ...
    /*std::cout << "n_alpha: " << vec_to_string(n_alpha) << std::endl;
    std::cout << "n_beta: " << vec_to_string(n_beta) << std::endl;
    std::cout << "n_gamma: " << vec_to_string(n_gamma) << std::endl;*/
...
/*MolCubeShell::MolCubeShell(Molecule *molecule, int partition, float precision) : mol(molecule) {
    part = partition;
    eps = precision;
    packed = false;
    ...
    *//*std::cout << "n_alpha: " << vec_to_string(n_alpha) << std::endl;
    ...
    plane_front.C_point = M;
    plane_bottom.C_point = M;
    *//*std::cout << "plane basis angles: " << glm::angle(plane_front.N_vec, plane_left.N_vec) << " "
        << glm::angle(plane_left.N_vec, plane_bottom.N_vec) << " "
        << glm::angle(plane_front.N_vec, plane_bottom.N_vec) << std::endl;*//*
}*/
...
//                std::cout << "current angles: " << alpha << " " << beta << " " << gamma << std::endl;
                ...
                if (cur_len < min_len) {
                    ...
                    proper_beta = beta;
                    proper_gamma = gamma;
                    std::cout << "box size: " << a << "x" << b << "x" << c << std::endl;
                    std::cout << "proper angles: " << alpha << " " << beta << " " << gamma << std::endl;
    ...
    if (min_len == 100) {
        std::cerr << "error: unable to pack mol in cube!" << std::endl;
        min_len = -1;
    }
...
std::vector<glm::vec3> MolCubeShell::getShellPoints() {
    ...
    std::cout << "min_len: " << cube_edge_len << std::endl;
...
bool MolCubeShell::intersectsWithSpheres() {
    /*std::cout << "--------------" << std::endl;
    if(plane_left.intersectsWithSphere(C, atom_c().vdw_radius))
    {
        std::cout << "intersection with left plane" << std::endl;
    }
    if(plane_bottom.intersectsWithSphere(C, atom_c().vdw_radius))
    {
        std::cout << "intersection with bottom plane" << std::endl;
    }
    if(plane_left.intersectsWithSphere(B, atom_b().vdw_radius))
    {
        std::cout << "intersection with left plane" << std::endl;
    }*/
...
void MolCubeShell::superposePlanesWithAxis() {
    ...
    float phi = -glm::angle(plane_front.n_vec, vec_i);
    glm::vec3 v_rot_phi = (glm::cross(plane_front.n_vec, vec_i));
    ...
    std::cout << "result angles between axes:" << glm::degrees(glm::angle(plane_front.N_vec, vec_i)) << " "
    << glm::degrees(glm::angle(plane_bottom.N_vec, vec_j)) << " "
    << glm::degrees(glm::angle(plane_left.N_vec, vec_k)) << std::endl;
    std::cout << "-------------------------" << std::endl;
}
...
std::vector<glm::vec3> MolCubeShell::getDisplacementVectors() {
    std::cout << "cube_edge_len: " << cube_edge_len << std::endl;
    if (displacementVectors.size() == 0) {
        displacementVectors.emplace_back(cube_edge_len*glm::normalize(plane_front.N_vec));
        displacementVectors.emplace_back(cube_edge_len*glm::normalize(plane_left.N_vec));
        displacementVectors.emplace_back(cube_edge_len*glm::normalize(plane_bottom.N_vec));
    }
