...
void MolCubeShell::pack() {
    ...
    if (min_len == 100) {
        std::cerr << "error: unable to packMax Molecule in cube!" << std::endl;
        min_len = -1;
    }
...
void MolCubeShell::initShell() {
    ...
    if (mol->AtomsCount() != 3) {
        ...
        std::cerr << "Error: unable to packMax Molecule. Atoms count should be 3" << std::endl;
        throw;
    }
...
void MolCubeShell::pack(MolCubeShell::compareCubeEdgeLes compFunc) {
    float alpha = 0;
    float beta = 0;
    float gamma = 0;
    ...
    float d_phi = 45.f/part;
    float cur_len, optimal_len;

    float a = get_shell_len_a();
    float b = get_shell_len_b();
    float c = get_shell_len_c();

    optimal_len = std::max(a, std::max(b, c));
    ...
    while(i < part && !intersectsWithSpheres()) {
        ...
        while(j < part && !intersectsWithSpheres()) {
            ...
            while(k < part && !intersectsWithSpheres()) {
                ...
                cur_len = std::max(a, std::max(b, c));
                if (compFunc(cur_len, optimal_len)) {
                    optimal_len = cur_len;
                    proper_alpha = alpha;
                    proper_beta = beta;
                    proper_gamma = gamma;
    ...
    packed = true;
    cube_edge_len = optimal_len;
    superposePlanesWithAxis();
}

float MolCubeShell::get_shell_len_a() {
    return std::max(
            plane_left.distanceFrom(atom_b().coord) + atom_b().vdw_radius,
            plane_left.distanceFrom(atom_c().coord) + atom_c().vdw_radius
    );
...
float MolCubeShell::get_shell_len_b() {
    return  std::max(
            plane_bottom.distanceFrom(atom_b().coord) + atom_b().vdw_radius,
            plane_bottom.distanceFrom(atom_c().coord) + atom_c().vdw_radius
    );
...
float MolCubeShell::get_shell_len_c() {
    return std::max(
            plane_front.distanceFrom(atom_b().coord) + atom_b().vdw_radius,
            plane_front.distanceFrom(atom_c().coord) + atom_c().vdw_radius
    );
