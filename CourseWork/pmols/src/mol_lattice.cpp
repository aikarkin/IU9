...
SimpleLattice::SimpleLattice(Molecule &mol_prototype, float length, float width, float height) : mol(mol_prototype) {
    this->length = length;
    this->width = width;
    this->height = height;
...
std::vector<Molecule> &SimpleLattice::getMolecules() {
    return mols;
}

void SimpleLattice::formLattice() {
...
void SimpleLattice::saveToFile(std::string output_file, std::string output_format) {
    if(output_format == "json") {
        rapidjson::Document jsonDoc;
        jsonDoc.SetObject();
        rapidjson::Value mols_arr(rapidjson::kArrayType);
...
//                std::cout << "json - bond # " << j << ": " << begin.GetString() << " --> " << end.GetInt() << std::endl;
    ...
    for (int i = 0; i < mols.size(); ++i) {
        ...
        FOR_ATOMS_OF_MOL(a, cur_mol) {
            mol_lattice->AddAtom(*a);
            std::cout << "atom_idx: " << a->GetIdx() << std::endl;
        }*/

        for (int j = 0; j < mols[i].AtomsCount(); ++j) {
            OpenBabel::OBAtom obAtom = mols[i].GetAtom(j).OBAtom();
            /*std::cout << "pmols.Atom - index: " << mols[i].GetAtom(j).atom_idx << std::endl;
            std::cout << "OBAtom - index: " << obAtom.GetIdx() << std::endl;*/
            mol_lattice->AddAtom(obAtom);
        }

        for (int j = 0; j < mols[i].BondsCount(); ++j) {
//            atom_b_idx = i * mols[i].AtomsCount() + mols[i].GetBond(j).begin->atom_idx;
//            atom_e_idx = i * mols[i].AtomsCount() + mols[i].GetBond(j).end->atom_idx;
            atom_b_idx = mols[i].GetBond(j).begin->atom_idx;
            atom_e_idx = mols[i].GetBond(j).end->atom_idx;
//            std::cout << atom_b_idx << " -> " << atom_e_idx << std::endl;
            mol_lattice->AddBond(atom_b_idx, atom_e_idx, 1);
        }
    }
